...
export function AdminOverviewPage() {
  ...
  return (
    <>
      ...
      <div className="dashboard-grid"><Card className="chart-card"><div className="card-heading"><div><h2>GMV và đơn hoàn tất</h2><p>Theo khoảng ngày đang chọn.</p></div></div><Suspense fallback={<SkeletonRows count={3} />}><AnalyticsChart options={chartOptions} series={[{ name: "GMV", type: "area", data: data.chart.map((point) => point.gmv) }, { name: "Đơn hàng", type: "line", data: data.chart.map((point) => point.orders) }]} height={340} /></Suspense></Card><Card><div className="card-heading"><div><h2>Top nhà hàng</h2><p>Xếp theo GMV đơn hoàn tất.</p></div></div><div className="rank-list">{data.topRestaurants.map((item, index) => <div key={item.restaurantId}><span>{index + 1}</span><div><b>{item.name}</b><small>{formatNumber(item.orders)} đơn · Hủy {item.cancellationRate.toFixed(1)}%</small></div><strong>{formatMoney(item.gmv)}</strong></div>)}</div></Card></div>
