...
export const RestaurantCard: React.FC<RestaurantCardProps> = ({
  ...
  return (
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-[#e7ece8] bg-white shadow-[0_5px_18px_rgba(34,63,43,.04)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_18px_50px_rgba(32,70,45,.10)]">
      {/* Image Header */}
      <div className="relative h-[190px] overflow-hidden bg-[#fff0e9]">
      ...
      <div className="flex flex-1 flex-col p-4">
        <Link to={`/restaurants/${restaurant.slug}`}>
          <h3 className="mb-1 text-base font-bold leading-snug text-[#17201a] transition group-hover:text-[#ff5a1f] line-clamp-1">
            {restaurant.name}
          </h3>
        </Link>

        {categoriesText && (
          <p className="mb-1 text-xs text-[#68736c] line-clamp-1">{categoriesText}</p>
        )}

        <p className="mb-3 text-xs text-[#68736c] line-clamp-1 flex items-center gap-1">
          <MapPin className="h-3.5 w-3.5 text-slate-400 shrink-0" />
          <span>{addressText}</span>
        </p>

        {/* Stats Row */}
        <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-[#68736c]">
        ...
        {/* Footer Row */}
        <div className="mt-auto flex items-center justify-between gap-3 pt-4 border-t border-[#e7ece8] text-xs">
          <span
            className={`${
          ...
          <Link
            ...
            className="flex items-center gap-0.5 font-bold text-[#ff5a1f] hover:text-[#e94e16] transition"
          >
            <span>Xem quán</span>
