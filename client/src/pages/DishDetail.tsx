...
export const DishDetail = () => {
  ...
  const [pendingCartItem, setPendingCartItem] = useState<NewCartItem | null>(null)
  ...
  const unitPrice = (item?.effectivePrice ?? 0) + selectedOptions.reduce((sum, option) => sum + option.priceDelta, 0)
  ...
  const toggleOption = (group: OptionGroup, optionId: string) => {
    ...
    setSelections((current) => {
      ...
      if (selected.includes(optionId)) {
        return { ...current, [group._id]: selected.filter((id) => id !== optionId) }
      }
  ...
  const commitCartItem = (newItem: NewCartItem, replaceDifferentRestaurant = false) => {
    if (!item) return
    addStoredCartItem(
      readStoredCart(),
      { id: item.restaurant._id, name: item.restaurant.name, slug: item.restaurant.slug },
      newItem,
      replaceDifferentRestaurant
    )
    setAddedMessage(`Đã thêm ${newItem.quantity} × ${item.name} vào giỏ.`)
  ...
  const addToCart = () => {
    ...
    if (invalidGroup) {
      setSelectionError(`Vui lòng chọn đủ tuỳ chọn trong nhóm “${invalidGroup.name}”.`)
      document.getElementById(`option-${invalidGroup._id}`)?.scrollIntoView({ behavior: 'smooth', block: 'center' })
      return
    }
    if (!isAuthenticated) {
      setShowLogin(true)
    ...
    if (isDifferentRestaurant(currentCart, item.restaurant._id)) {
      setCurrentRestaurantName(currentCart?.restaurantName ?? 'quán hiện tại')
    ...
    return (
      <main className="grid min-h-[70vh] place-items-center bg-[#f7faf7] px-5 py-16 text-center">
        <div className="max-w-lg rounded-3xl border border-[#e7ece8] bg-white px-8 py-12">
          ...
          <Link to={`/restaurants/${restaurantSlug}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#ff5a1f] px-5 font-bold text-white hover:bg-[#e94e16]">
  ...
  const images = item.imageUrls.length > 0 ? item.imageUrls : ['/assets/noodles.jpg']
  ...
  return (
    <main className="min-h-screen bg-[#f7faf7] text-[#17201a]">
      <section className="border-b border-[#e7ece8] bg-[#eef8f1] py-12 max-[760px]:py-8">
        <div className="mx-auto w-[calc(100%-2.5rem)] max-w-[1180px] max-[760px]:w-[calc(100%-1.5rem)]">
          <div className="mb-8 flex flex-wrap items-center gap-2 text-xs text-[#68736c]">
            <Link to="/restaurants" className="hover:text-[#ff5a1f]">Nhà hàng</Link>
            <span>/</span>
            <Link to={`/restaurants/${item.restaurant.slug}`} className="hover:text-[#ff5a1f]">{item.restaurant.name}</Link>
            <span>/</span>
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] items-start gap-14 max-[850px]:grid-cols-1">
            <div>
              <div className="overflow-hidden rounded-3xl border border-[#dce8df] bg-white">
                <img src={images[activeImage]} alt={item.name} className="h-[500px] w-full object-cover max-[600px]:h-[340px]" />
              </div>
              {images.length > 1 && (
            ...
            <div className="rounded-3xl border border-[#dce8df] bg-white p-7 max-[600px]:p-5">
              ...
              <h1 className="mb-3 text-[40px] font-black leading-tight tracking-[-.04em] max-[600px]:text-[32px]">{item.name}</h1>
              <p className="mb-4 leading-7 text-[#68736c]">{item.description || item.shortDescription}</p>
              <div className="mb-6 flex items-end gap-3 border-b border-[#e7ece8] pb-6">
                <strong className="text-3xl font-black text-[#ff5a1f]">{formatMoney(item.effectivePrice)}</strong>
                {item.salePrice !== null && <span className="pb-1 text-sm text-[#9ca59f] line-through">{formatMoney(item.basePrice)}</span>}
                <span className="ml-auto pb-1 text-xs text-[#68736c]">Đã bán {new Intl.NumberFormat('vi-VN').format(item.soldCount)}</span>
              </div>

              {item.ingredients.length > 0 && (
                <div className="mb-6 rounded-2xl bg-[#f7faf7] p-4">
                  <b className="mb-2 block text-xs uppercase tracking-wide">Thành phần</b>
                  <p className="mb-0 text-xs leading-6 text-[#68736c]">{item.ingredients.join(' · ')}</p>
                </div>
              )}

              {item.optionGroups.map((group) => (
                <fieldset id={`option-${group._id}`} key={group._id} className="mb-6 scroll-mt-32 border-0 p-0">
                  <legend className="mb-3 flex w-full items-center justify-between gap-3 text-sm font-extrabold">
                    <span>{group.name}</span>
                    <span className="text-[10px] font-bold text-[#68736c]">{group.required ? 'Bắt buộc' : 'Tuỳ chọn'} · chọn tối đa {group.maxSelect}</span>
                  </legend>
                  <div className="grid gap-2">
                    {group.options.map((option) => {
                      const checked = selections[group._id]?.includes(option._id) ?? false
                      return (
                        <label key={option._id} className={`flex cursor-pointer items-center justify-between gap-4 rounded-xl border p-3.5 transition ${checked ? 'border-[#ff5a1f] bg-[#fff7f3]' : 'border-[#e7ece8] bg-white hover:border-[#ffc4ad]'}`}>
                          ...
                          <b className="text-xs">{option.priceDelta > 0 ? `+${formatMoney(option.priceDelta)}` : '0đ'}</b>
                        </label>
                      )
                    })}
                  </div>
              ...
              <label className="mb-6 block">
                <span className="mb-2 block text-xs font-extrabold">Ghi chú cho quán</span>
                <textarea
                  value={note}
                ...
                />
                <span className="mt-1 block text-right text-[10px] text-[#9ca59f]">{note.length}/200</span>
              </label>
              ...
              <div className="flex gap-3 max-[480px]:flex-col">
                ...
                >
                  ...
                  {item.isAvailable ? `Thêm vào giỏ · ${formatMoney(totalPrice)}` : 'Món đang tạm hết'}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto w-[calc(100%-2.5rem)] max-w-[1180px] max-[760px]:w-[calc(100%-1.5rem)]">
            ...
            <div className="grid grid-cols-4 gap-5 max-[900px]:grid-cols-2 max-[520px]:grid-cols-1">
              {item.relatedItems.map((related) => (
                <Link key={related._id} to={`/restaurants/${item.restaurant.slug}/menu-items/${related.slug}`} className="group overflow-hidden rounded-2xl border border-[#e7ece8] bg-white transition hover:border-[#ffc4ad]">
                  <div className="relative h-40 overflow-hidden bg-[#eef8f1]">
                    <img src={related.imageUrl || '/assets/noodles.jpg'} alt={related.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-105" />
                    {!related.isAvailable && <span className="absolute inset-0 grid place-items-center bg-slate-900/50 text-xs font-extrabold text-white">Tạm hết món</span>}
                  </div>
                  <div className="p-4">
                    <h3 className="mb-2 truncate text-sm font-bold group-hover:text-[#ff5a1f]">{related.name}</h3>
                    <div className="flex items-center gap-2">
                      <b className="text-[#ff5a1f]">{formatMoney(related.effectivePrice)}</b>
                      {related.salePrice !== null && <span className="text-[10px] text-[#9ca59f] line-through">{formatMoney(related.basePrice)}</span>}
                    </div>
                  </div>
                </Link>
              ))}
      ...
      <LoginModal isOpen={showLogin} onClose={() => setShowLogin(false)} itemName={item.name} />
      <RestaurantChangeModal
        isOpen={Boolean(pendingCartItem)}
        currentRestaurantName={currentRestaurantName}
        nextRestaurantName={item.restaurant.name}
        onCancel={() => setPendingCartItem(null)}
        onConfirm={() => {
          if (pendingCartItem) commitCartItem(pendingCartItem, true)
