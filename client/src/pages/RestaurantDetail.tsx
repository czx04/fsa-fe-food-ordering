...
export const RestaurantDetail = () => {
  ...
  const [reviewPagination, setReviewPagination] = useState<ReviewResponse['pagination']>({
    page: 1,
    limit: 5,
    total: 0,
    totalPages: 0,
  ...
  const [storedCart, setStoredCart] = useState<StoredCart | null>(() => readStoredCart())
  const [pendingMenuItem, setPendingMenuItem] = useState<MenuItem | null>(null)
  const [loginItem, setLoginItem] = useState('')

  useEffect(() => {
    const syncCart = () => setStoredCart(readStoredCart())
    window.addEventListener('storage', syncCart)
    window.addEventListener('mammam-cart-updated', syncCart)
    return () => {
      window.removeEventListener('storage', syncCart)
      window.removeEventListener('mammam-cart-updated', syncCart)
    }
  }, [])
    ...
    const loadRestaurant = async () => {
      ...
      try {
        const [restaurantResponse, categoriesResponse] = await Promise.all([
          api.get<{ data: RestaurantDetailData }>(`/public/restaurants/${restaurantSlug}`),
  ...
  useEffect(() => {
    ...
    const loadMenu = async () => {
      ...
      setMenuError('')
      try {
        const params = new URLSearchParams({ limit: '50', availability: 'all' })
  ...
  useEffect(() => {
    ...
    const loadReviews = async () => {
      ...
      setReviewsError('')
      try {
        const params = new URLSearchParams({ page: String(reviewPage), limit: '5', sort: 'newest' })
  ...
  const currentRestaurantCart =
    restaurant && storedCart?.restaurantId === restaurant._id ? storedCart : null
  const cartItems = currentRestaurantCart?.items ?? []
  const cartQuantity = cartItems.reduce((total, entry) => total + entry.quantity, 0)
  ...
  const createQuickCartItem = (item: MenuItem): NewCartItem => ({
    itemId: item._id,
    name: item.name,
  ...
  const addItem = (item: MenuItem) => {
    ...
    if (restaurant && isDifferentRestaurant(readStoredCart(), restaurant._id)) {
      setPendingMenuItem(item)
      return
    }
    commitQuickItem(item)
  }

  const changeQuantity = (itemKey: string, delta: number) => {
    if (!currentRestaurantCart) return
    setStoredCart(changeStoredCartQuantity(currentRestaurantCart, itemKey, delta))
  }
    ...
    return (
      <main className="grid min-h-[70vh] place-items-center bg-[#f7faf7] px-5 py-16 text-center">
        <div className="max-w-lg rounded-3xl border border-[#e7ece8] bg-white px-8 py-12 shadow-sm">
          ...
          <h1 className="mb-3 text-3xl font-extrabold text-[#17201a]">Không tìm thấy quán</h1>
  ...
  const deliveryTotal = cartSubtotal + (cartItems.length > 0 ? restaurant.delivery.fee : 0)
  ...
  return (
    <main className="min-h-screen bg-[#f7faf7] text-[#17201a]">
            ...
            <div>
              ...
              <div className="mb-3 flex flex-wrap items-center gap-2">
                ...
                {restaurant.cuisineCategories.map((category) => (
                  <span key={category._id} className="rounded-full bg-white/15 px-3 py-1.5 text-[11px] font-bold backdrop-blur">
                    {category.name}
                  </span>
                ))}
              </div>
              <div className="mb-4 flex items-center gap-4">
                <img
                  src={restaurant.logoUrl || restaurant.coverUrl || '/assets/restaurant.jpg'}
              ...
              <div className="flex flex-wrap gap-x-7 gap-y-2.5 text-xs font-semibold text-white/90">
                <span className="inline-flex items-center gap-1.5">
                  ...
                  {summary.average.toFixed(1)} ({formatCount(summary.count)} đánh giá)
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <MapPin className="h-4 w-4" /> {restaurant.address.formatted}
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <Clock3 className="h-4 w-4" /> {restaurant.delivery.minMinutes}–{restaurant.delivery.maxMinutes} phút
                </span>
              </div>
            </div>
            <button
          ...
          {[
            ...
            <button
              ...
              className={`shrink-0 border-0 border-b-[3px] bg-transparent px-0 py-4 text-xs font-bold ${
                index === 0 ? 'border-[#ff5a1f] text-[#ff5a1f]' : 'border-transparent text-[#68736c]'
              }`}
            >
              {label}
            </button>
        ...
        <div className="mx-auto grid w-[calc(100%-2.5rem)] max-w-[1180px] grid-cols-[minmax(0,1fr)_330px] items-start gap-8 max-[900px]:grid-cols-1 max-[760px]:w-[calc(100%-1.5rem)]">
          <div className="min-w-0">
            <div className="mb-6 flex items-end justify-between gap-5 max-[760px]:flex-col max-[760px]:items-stretch">
              ...
              <form onSubmit={submitMenuSearch} className="flex min-w-[280px] items-center rounded-xl border border-[#e7ece8] bg-white px-3 focus-within:border-[#ff5a1f] max-[760px]:min-w-0">
            ...
            {!menuLoading && !menuError && menuItems.length > 0 && (
              <div className="grid gap-3.5">
                {menuItems.map((item) => (
                  ...
                  >
                    <Link to={`/restaurants/${restaurant.slug}/menu-items/${item.slug}`} className="relative block overflow-hidden rounded-xl">
                      <img
                    ...
                    <div className="min-w-0">
                      <div className="mb-1 flex flex-wrap items-center gap-2">
                        <Link to={`/restaurants/${restaurant.slug}/menu-items/${item.slug}`} className="min-w-0">
                          <h3 className="mb-0 truncate text-[15px] font-bold transition hover:text-[#ff5a1f]">{item.name}</h3>
                        </Link>
                        {item.soldCount > 100 && (
                      ...
                      <p className="mb-2 line-clamp-2 text-xs text-[#68736c]">{item.shortDescription}</p>
                      <div>
                        <b className="text-[16px] text-[#ff5a1f]">{formatMoney(item.effectivePrice)}</b>
                        {item.salePrice !== null && (
            ...
            <section id="about" className="scroll-mt-32 pt-14">
              ...
              <div className="rounded-2xl border border-[#e7ece8] bg-white p-6 shadow-sm">
                <p className="mb-6 leading-7 text-[#68736c]">{restaurant.description}</p>
                <div className="grid grid-cols-2 gap-4 max-[600px]:grid-cols-1">
                  <div className="flex gap-3 rounded-xl bg-[#f7faf7] p-4">
                    <MapPin className="mt-0.5 h-5 w-5 shrink-0 text-[#ff5a1f]" />
                    <div><b className="block text-xs">Địa chỉ</b><span className="text-xs text-[#68736c]">{restaurant.address.formatted}</span></div>
                  </div>
                  <a href={`tel:${restaurant.phone}`} className="flex gap-3 rounded-xl bg-[#f7faf7] p-4 transition hover:bg-[#fff0e9]">
                    <Phone className="mt-0.5 h-5 w-5 shrink-0 text-[#ff5a1f]" />
                    <div><b className="block text-xs">Điện thoại</b><span className="text-xs text-[#68736c]">{restaurant.phone}</span></div>
                  </a>
                </div>
              </div>
            </section>
            ...
            <section id="reviews" className="scroll-mt-32 pt-14">
              <div className="mb-6 flex items-end justify-between gap-4 max-[600px]:items-start">
                <div>
                  ...
                  <p className="mb-0 text-xs text-[#68736c]">
                    Điểm trung bình {summary.average.toFixed(1)} từ {formatCount(summary.count)} đánh giá
                  </p>
                </div>
              </div>
              ...
              <div className="mb-5 grid grid-cols-[170px_1fr] gap-7 rounded-2xl border border-[#e7ece8] bg-white p-6 max-[600px]:grid-cols-1">
                <div className="grid place-items-center border-r border-[#e7ece8] text-center max-[600px]:border-b max-[600px]:border-r-0 max-[600px]:pb-5">
                  <strong className="text-5xl font-black tracking-[-.05em]">{summary.average.toFixed(1)}</strong>
                  <div className="my-2 flex gap-0.5 text-amber-400" aria-label={`${summary.average} trên 5 sao`}>
                    {Array.from({ length: 5 }, (_, index) => (
                      <Star key={index} className={`h-4 w-4 ${index < Math.round(summary.average) ? 'fill-current' : ''}`} />
                    ))}
                  </div>
                  <span className="text-[11px] text-[#68736c]">{formatCount(summary.count)} lượt đánh giá</span>
                </div>
                <div className="grid gap-2">
                  {[5, 4, 3, 2, 1].map((rating) => {
                    const count = summary.distribution[String(rating) as keyof RatingSummary['distribution']] ?? 0
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setReviewRating((current) => (current === rating ? null : rating))
                          setReviewPage(1)
                        }}
                      ...
                      >
                        ...
                        <span className="h-2 overflow-hidden rounded-full bg-[#eef2ef]">
                          <span className="block h-full rounded-full bg-amber-400" style={{ width: `${percentage}%` }} />
                        </span>
                        <span className="text-right text-[#68736c]">{formatCount(count)}</span>
                      </button>
                    )
                  })}
                </div>
              ...
              {reviewsLoading && <div className="rounded-2xl border border-[#e7ece8] bg-white p-10 text-center text-[#68736c]">Đang tải đánh giá...</div>}
              {reviewsError && <div className="rounded-2xl border border-red-200 bg-red-50 p-8 text-center text-red-600">{reviewsError}</div>}
              {!reviewsLoading && !reviewsError && reviews.length === 0 && (
                <div className="rounded-2xl border border-[#e7ece8] bg-white p-10 text-center text-[#68736c]">
                  {reviewRating ? `Chưa có đánh giá ${reviewRating} sao.` : 'Nhà hàng chưa có đánh giá công khai.'}
                </div>
              )}
              {!reviewsLoading && !reviewsError && reviews.length > 0 && (
                <div className="overflow-hidden rounded-2xl border border-[#e7ece8] bg-white px-6">
                  {reviews.map((review) => (
                    <article key={review._id} className="grid grid-cols-[44px_1fr] gap-3.5 border-b border-[#e7ece8] py-5 last:border-b-0">
                      ...
                      <div>
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          <span className="text-[11px] text-[#9ca59f]">{formatDate(review.createdAt)}</span>
                        </div>
                        ...
                        <p className="mb-0 text-sm leading-6 text-[#526158]">{review.content}</p>
                        {review.ownerReply && (
                          <div className="mt-3 rounded-xl bg-[#f7faf7] p-3 text-xs text-[#68736c]">
                            <b className="mb-1 block text-[#17201a]">Phản hồi từ {restaurant.name}</b>
              ...
              {reviewPagination.totalPages > 1 && (
                <div className="mt-5 flex items-center justify-center gap-2">
                  ...
                  <span className="px-3 text-xs font-bold">{reviewPage} / {reviewPagination.totalPages}</span>
                  <button
                    type="button"
          ...
          <aside className="sticky top-[138px] overflow-hidden rounded-2xl border border-[#e7ece8] bg-white shadow-[0_8px_24px_rgba(34,63,43,.08)] max-[900px]:static">
            <div className="border-b border-[#e7ece8] p-5">
              <div className="mb-1 flex items-center justify-between gap-3">
                ...
                {cartQuantity > 0 && <span className="rounded-full bg-[#fff0e9] px-2.5 py-1 text-[10px] font-extrabold text-[#ff5a1f]">{cartQuantity} món</span>}
              </div>
              <p className="mb-0 text-[11px] text-[#68736c]">{restaurant.name}</p>
            </div>
            <div className="p-5">
              {storedCart && storedCart.items.length > 0 && storedCart.restaurantId !== restaurant._id && (
                <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[11px] leading-5 text-amber-800">
                  Giỏ hàng hiện có {storedCart.items.reduce((sum, entry) => sum + entry.quantity, 0)} món từ{' '}
                  <b>{storedCart.restaurantName}</b>. Khi chọn món ở đây, hệ thống sẽ hỏi trước khi đổi quán.
                </div>
              )}
              {cartItems.length === 0 ? (
                ...
                <div className="mb-4 max-h-[280px] overflow-y-auto pr-1">
                  {cartItems.map((cartItem) => (
                    <div key={cartItem.key} className="border-b border-dashed border-[#e7ece8] py-3 first:pt-0">
                      <div className="mb-2 flex items-start justify-between gap-3 text-xs">
                        <span className="font-bold">{cartItem.name}</span>
                        <b className="shrink-0">{formatMoney(cartItem.unitPrice * cartItem.quantity)}</b>
                      </div>
                      ...
                      <div className="inline-flex items-center overflow-hidden rounded-lg border border-[#e7ece8]">
                        <button type="button" onClick={() => changeQuantity(cartItem.key, -1)} className="grid h-7 w-7 place-items-center bg-white"><Minus className="h-3 w-3" /></button>
                        <span className="grid h-7 min-w-7 place-items-center text-[11px] font-bold">{cartItem.quantity}</span>
                        <button type="button" onClick={() => changeQuantity(cartItem.key, 1)} className="grid h-7 w-7 place-items-center bg-white"><Plus className="h-3 w-3" /></button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="mb-4 rounded-xl bg-[#f7faf7] p-3 text-xs">
                <div className="mb-2 flex justify-between text-[#68736c]"><span>Tạm tính</span><b className="text-[#17201a]">{formatMoney(cartSubtotal)}</b></div>
                <div className="mb-3 flex justify-between text-[#68736c]"><span>Phí giao hàng</span><b className="text-[#17201a]">{restaurant.delivery.fee === 0 ? 'Miễn phí' : formatMoney(restaurant.delivery.fee)}</b></div>
                <div className="flex justify-between border-t border-[#e7ece8] pt-3 text-sm"><b>Tổng cộng</b><b className="text-[#ff5a1f]">{formatMoney(deliveryTotal)}</b></div>
              </div>
              <Link
            ...
            <div className="border-t border-[#e7ece8] bg-[#fbfdfb] p-5">
              ...
              <div className="mt-4 grid gap-1.5 border-t border-[#e7ece8] pt-4 text-[11px]">
                {restaurant.openingHours.map((entry) => (
                  <div key={entry.dayOfWeek} className="flex justify-between gap-3 text-[#68736c]">
                    <span>{DAY_NAMES[entry.dayOfWeek]}</span>
                    <b className="text-[#17201a]">{entry.isClosed || entry.slots.length === 0 ? 'Đóng cửa' : entry.slots.map((slot) => `${slot.open}–${slot.close}`).join(', ')}</b>
                  </div>
                ))}
              </div>
            </div>
      ...
      <LoginModal isOpen={Boolean(loginItem)} onClose={() => setLoginItem('')} itemName={loginItem} />
      <RestaurantChangeModal
        isOpen={Boolean(pendingMenuItem)}
        currentRestaurantName={storedCart?.restaurantName ?? 'quán hiện tại'}
        nextRestaurantName={restaurant.name}
        onCancel={() => setPendingMenuItem(null)}
        onConfirm={() => {
          if (pendingMenuItem) commitQuickItem(pendingMenuItem, true)
